...
void PreProcessing::calcExtremalValues(){
    if (analysis == RD){
        set<string> res;
        for (unsigned int i = 0; i<blocks.size(); i++){
            set<string> fvPerBlock = getFreeVariables(blocks[i]);
            vector<string> fvPerBlockVec (fvPerBlock.begin(), fvPerBlock.end());
            for (unsigned int i = 0; i<fvPerBlockVec.size(); i++){
                fvPerBlockVec[i] += ",?";
            }
            set<string> fvPerBlockMod (fvPerBlockVec.begin(), fvPerBlockVec.end());
        ...
        for (unsigned int i = 0; i<blocks.size(); i++){
            set<string> fvPerBlock = getFreeVariables(blocks[i]);
            set_union(res.begin(),res.end(), fvPerBlock.begin(), fvPerBlock.end(),inserter(res,res.begin()));
        }
        vector<string> resVec (res.begin(), res.end());
        for (unsigned int i = 0; i<resVec.size(); i++){
            extremalValuesSDA[resVec[i]] = {'-','0','+'};
        }
    }
}
...
set<string> PreProcessing::calcKillsForBlock (flgnode block){
    ...
    if (analysis == RD){
        vector<string> tokens = tokensAtLabels[block.getLabel()];
        //cout << "cur expr: " << block.getVal() << endl;
        bool isAssignment = false;
        string varAssTo = "";
        ...
        for (unsigned int i = 0; i<tokens.size(); i++){
            if (tokens[i] == "="){
                ...
                isAssignment = true;
                varAssTo = tokens[i+1];
                if ((i+2) <= tokens.size()-1) nextTok = tokens[i+2];
                break;
            }
        }
...
map<string,set<char>> PreProcessing::getExtremalValuesSDA(){
    return extremalValuesSDA;
}

map<int,vector<string>> PreProcessing::getTokensAtLabels(){
    return tokensAtLabels;
}
